...
const Differo = function ({ browser, imageWriter } = {}) {
  ...
  const diff = async ({ url, name }) => {
    const image = await screenshot({ url, name, write: false });
    await imageWriter.write({ image, name, overrideSuffix: NEW_SUFFIX });
    const diffImage = image; // FIXME Generate diff
    await imageWriter.write({ image: diffImage, name, overrideSuffix: DIFF_SUFFIX });
  };
